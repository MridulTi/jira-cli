...
Usage:
  ...
  jira.py log "Rough note" --time 2h       # Cursor fills title + 1–2 line description (default)
...
async def cmd_log(
    ...
    if use_cursor:
        print("Cursor: deriving title + short description…\n")
        try:
...
def parse_cursor_issue_payload(raw: str) -> tuple[str, str]:
    """Expect JSON {summary, description}; return trimmed title + 1–2 line description."""
...
def expand_issue_via_cursor(
    ...
    """
    Ask Cursor agent for a concise Jira summary line + 1–2 sentence description.
...
{{"summary": "<concise issue title, <= 90 characters>", "description": "<exactly 1 or 2 short sentences, plain text only>"}}

Rules:
- summary: clear title / outcome for the issue list (infer from fragments if needed).
- description: at most two sentences; factual; do not invent teams, tickets, or systems not hinted in the notes.
...
def build_parser() -> argparse.ArgumentParser:
    ...
    log_p = sub.add_parser(
        ...
        epilog=(
            "By default, Cursor CLI derives a concise issue title + 1–2 line description from your note.\n"
