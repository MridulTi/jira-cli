...
Usage:
  ...
  jira.py parent set TMD-100                    # weekly parent issue (Mon–Sat, stored in week.json + config)
  jira.py parent                                # show current week parent
  jira.py comment TMD-123 "Shipped fix"         # add comment to an issue
  jira.py eod                                   # per-issue: open / comment / change status
...
DEFAULT_CONFIG = {
    ...
    # Mirror of this week's parent (authoritative copy is week.json parentKey).
    "jiraWeekParentKey": "",
    # Per-tool timeout for MCP calls (seconds); avoids hanging forever on create/worklog.
    "jiraMcpToolTimeoutSec": 180,
    # Cache getJiraIssueTypeMetaWithFields pages under stateDir (hours; 0 = disable).
...
def _normalize_customfield_option_payloads(
    ...
    for key in list(merged.keys()):
        ...
        if isinstance(spec, dict):
            use_array = _field_schema_is_array(spec)
        else:
            # Without metadata: single-select CFs use one object; multi-value keeps a list.
            use_array = was_list and len(items) > 1
        if was_list and len(items) == 1 and not use_array:
            merged[key] = items[0]
        elif use_array or was_list:
            merged[key] = items
        else:
            merged[key] = items[0]
...
def _mcp_tool_timeout_sec(cfg: dict[str, Any]) -> float:
    raw = cfg.get("jiraMcpToolTimeoutSec", 180)
    try:
...
def _customfields_use_option_ids(merged: dict[str, Any]) -> bool:
    """True when every customfield_* value is already an option id (no label lookup needed)."""
    saw_cf = False
    for key, raw in merged.items():
        ...
        for item in items:
            if not isinstance(item, dict) or item.get("id") in (None, ""):
                return False
    return saw_cf or not any(str(k).startswith("customfield_") for k in merged)


def _fields_metadata_cache_path(
    cfg: dict[str, Any], cloud_id: str, project_key: str, issue_type_name: str
) -> Path:
    safe = re.sub(r"[^A-Za-z0-9._-]+", "_", f"{cloud_id}-{project_key}-{issue_type_name}")
    return Path(cfg["stateDir"]) / "cache" / f"fields-{safe}.json"


def _load_fields_metadata_cache(
    cfg: dict[str, Any], cloud_id: str, project_key: str, issue_type_name: str
) -> dict[str, Any] | None:
    hours = cfg.get("jiraCacheFieldMetadataHours", 24)
    try:
        hours_f = float(hours)
    except (TypeError, ValueError):
        hours_f = 24.0
    if hours_f <= 0:
        return None
    path = _fields_metadata_cache_path(cfg, cloud_id, project_key, issue_type_name)
    if not path.exists():
        return None
    try:
        blob = json.loads(path.read_text(encoding="utf-8"))
        fetched = blob.get("fetchedAt", "")
...
def _save_fields_metadata_cache(
    cfg: dict[str, Any],
    cloud_id: str,
    project_key: str,
    issue_type_name: str,
...
async def fetch_issue_type_fields_map_for_log(
    cfg: dict[str, Any],
    session: Any,
    cloud_id: str,
    project_key: str,
    ...
    """Field metadata for log creates; skips or caches when config already has option ids."""
    if cfg.get("jiraLogSkipFieldMetadata"):
        log_step("  Skipping field metadata (jiraLogSkipFieldMetadata).")
        return {}
    if not cfg.get("jiraResolveCustomfieldOptions", True):
        return {}
    cached = _load_fields_metadata_cache(cfg, cloud_id, project_key, issue_type_name)
    if cached is not None:
        log_step(f"  Using cached field metadata ({len(cached)} fields).")
        return cached

    if cfg.get("jiraLogSkipFieldMetadataWhenIdsResolved", True) and _customfields_use_option_ids(
        extra_fields
    ):
        log_step(
            "  Fetching field metadata once (needed for select-field shape: object vs array)."
        )

    timeout = _mcp_tool_timeout_sec(cfg)
    log_step("  Loading Jira field metadata (first run can take 1–2 min)…")
    ...
    _save_fields_metadata_cache(cfg, cloud_id, project_key, issue_type_name, fields_map)
    log_step(f"  Loaded {len(fields_map)} fields.")
    return fields_map
...
def normalize_issue_key(key: str) -> str:
    k = key.strip().upper()
    if not re.match(r"^[A-Z][A-Z0-9]+-\d+$", k):
        raise ValueError(f"Invalid issue key: {key!r}")
    return k


def get_week_parent(cfg: dict[str, Any]) -> str | None:
    """Parent issue for the current Mon–Sat week (from week.json)."""
    state = load_week_log(cfg)
    pk = str(state.get("parentKey") or "").strip().upper()
    return pk or None


def set_week_parent(cfg: dict[str, Any], key: str) -> str:
    """Set parent for this work week; resets automatically next Monday."""
    ...
    save_config({"jiraWeekParentKey": k})
    return k


def clear_week_parent(cfg: dict[str, Any]) -> None:
    state = load_week_log(cfg)
    state["parentKey"] = ""
    save_week_log(cfg, state)
    save_config({"jiraWeekParentKey": ""})
...
async def call_tool(
    ...
    *,
    ...
    sec = 180.0 if timeout_sec is None else max(30.0, float(timeout_sec))
    ...
    try:
        return await asyncio.wait_for(_invoke(), timeout=sec)
    except asyncio.TimeoutError as err:
        raise RuntimeError(
            f"MCP tool {name!r} timed out after {int(sec)}s. "
            "Retry, or raise jiraMcpToolTimeoutSec in ~/.config/jira-cli/config.json."
        ) from err


async def resolve_cloud_id(
    session: Any, cloud_id: str, *, timeout_sec: float | None = None
) -> str:
    if cloud_id:
        return cloud_id
    resources = await call_tool(
        session, "getAccessibleAtlassianResources", {}, timeout_sec=timeout_sec
    )
...
async def cmd_log(
    ...
    mcp_url = cfg.get("mcpUrl") or MCP_URL_DEFAULT
    mcp_timeout = _mcp_tool_timeout_sec(cfg)
    log_step("Jira: connecting via MCP (npx mcp-remote; OAuth if needed)…")
    async with mcp_session(mcp_url) as session:
        log_step("Jira: authenticating…")
        cloud_id = await resolve_cloud_id(
            session, str(cfg.get("cloudId") or ""), timeout_sec=mcp_timeout
        )
        user = await call_tool(
            session, "atlassianUserInfo", {}, timeout_sec=mcp_timeout
        )
        ...
        if cfg.get("jiraResolveCustomfieldOptions", True):
            fields_map = await fetch_issue_type_fields_map_for_log(
                cfg, session, cloud_id, project_key, type_name, extra_fields
            )
        ...
        parent_key = get_week_parent(cfg)
        if parent_key:
            create_payload["parent"] = parent_key

        log_step(
            f"Jira: creating {project_key} {type_name}"
            + (f" under {parent_key}…" if parent_key else "…")
        )
        created = await call_tool(
            session, "createJiraIssue", create_payload, timeout_sec=mcp_timeout
        )
        ...
        log_step(f"Jira: logging work ({jira_time})…")
        try:
            await call_tool(
                session, "addWorklogToJiraIssue", wl_args, timeout_sec=mcp_timeout
            )
...
async def cmd_parent(cfg: dict[str, Any], key: str | None, clear: bool) -> None:
    if clear:
        clear_week_parent(cfg)
        print("Cleared weekly parent issue.")
        return
    if not key:
        parent = get_week_parent(cfg)
        state = load_week_log(cfg)
        print(f"Week {state.get('week_start')} – {state.get('week_end_sat')} (Mon–Sat)")
        if parent:
            print(f"  Parent: {parent}")
            print(issue_url(cfg, parent))
            print(f"  (also in config: jiraWeekParentKey)")
        else:
            print("  Parent: (not set)")
            print("  Set with: jira parent set TMD-123")
        return
    k = set_week_parent(cfg, key)
    print(f"Weekly parent set to {k} (until next Monday–Saturday week)")
    print(issue_url(cfg, k))


async def cmd_comment(cfg: dict[str, Any], key: str, body: str) -> None:
    mcp_url = cfg.get("mcpUrl") or MCP_URL_DEFAULT
    k = normalize_issue_key(key)
    text = body.strip()
    if not text:
        raise RuntimeError("Comment body cannot be empty.")
    async with mcp_session(mcp_url) as session:
        cloud_id = await resolve_cloud_id(session, str(cfg.get("cloudId") or ""))
        await call_tool(
            session,
...
def _resolve_picked_issue(
    raw: str, numbered: list[dict[str, str]]
) -> str | None:
    ...
    if part.isdigit():
        ...
        if 0 <= idx < len(numbered):
            return numbered[idx]["key"]
        return None
    key_set = {row["key"].upper() for row in numbered}
    if part in key_set:
        return part
    return None


async def _eod_issue_actions(
    session: Any,
    cloud_id: str,
    cfg: dict[str, Any],
    key: str,
    ...
    """Per-issue menu: open, comment, or change status."""
    label = row.get("summary", key)
    st = row.get("status")
    header = f"\n{key} — {label}" + (f"  [{st}]" if st else "")
    print(header)
    print("  1. Open in browser")
    print("  2. Add comment")
    print("  3. Change status")
    print("  4. Back (next issue)")
    raw = input("Choose [1-4]: ").strip()
    if raw == "1":
        open_issue_in_browser(cfg, key)
        return
    if raw == "2":
    ...
    if raw == "3":
        ...
        try:
            ...
            print(f"  {key} → {to}")
            if _is_eod_excluded_status(to, _eod_excluded_statuses(cfg)):
                remove_week_log_entry(cfg, key)
                print(f"  Removed {key} from local week log (closed status).")
        except RuntimeError as err:
            print(f"  {err}", file=sys.stderr)
        return
    if raw != "4":
        print("  Unknown choice — back to list.")


async def _run_eod_interactive(
    session: Any,
    cloud_id: str,
    cfg: dict[str, Any],
    ...
    print("\nEnter issue # or key to act on (open / comment / status). Empty line to finish.")
    while True:
        raw = input("> ").strip()
        if not raw:
            break
        key = _resolve_picked_issue(raw, numbered)
...
async def cmd_eod(
    ...
    async with mcp_session(mcp_url) as session:
        ...
        if list_only:
            ...
            print("\nUse: jira eod  (open / comment / status per issue)  |  jira eod --done KEY [--status DONE]")
            return
        ...
        await _run_eod_interactive(session, cloud_id, cfg, issues)
...
- No keys other than summary and description."""
    ...
    if proc.returncode != 0:
        ...
        if proc.returncode == -2:
            raise RuntimeError(
                "interrupted (Ctrl+C) — wait for Cursor to finish or use --plain"
            )
...
def parse_parent_tokens(tokens: list[str]) -> tuple[str, str | None]:
    """Return (action, key) where action is show | set | clear."""
    if not tokens:
        return "show", None
    if tokens[0] == "clear":
        return "clear", None
    if tokens[0] == "set":
        if len(tokens) < 2:
            raise SystemExit("usage: jira parent set KEY  (e.g. jira parent set TMD-100)")
        return "set", tokens[1]
    return "set", tokens[0]


def cmd_doctor() -> None:
    """Print which jira.py is running and whether this build has recent commands."""
    script = Path(__file__).resolve()
    text = script.read_text(encoding="utf-8")
    line_count = text.count("\n") + (0 if text.endswith("\n") else 1)
    markers = {
        "parent command": "async def cmd_parent",
        "comment command": "async def cmd_comment",
        "interactive eod": "async def _run_eod_interactive",
        "week parent in week.json": "parentKey",
    }
    print(f"jira-cli {CLI_VERSION}")
    ...
    which = shutil.which("jira") or "(not on PATH)"
    print(f"PATH jira: {which}")
    dotfiles = Path.home() / "dotfiles" / "jira-cli" / "jira.py"
    if dotfiles.exists() and dotfiles.resolve() != script:
        same = dotfiles.read_bytes() == script.read_bytes()
        print(f"dotfiles copy: {dotfiles}  ({'same bytes' if same else 'DIFFERS — run ~/dotfiles/jira-cli/sync-install.sh'})")
    if line_count < 1900:
        print(
            "\nThis build looks like an old install (GitHub main is ~1816 lines). "
            "Re-running setup.sh from GitHub can overwrite your copy. "
            "Sync from dotfiles instead: ~/dotfiles/jira-cli/sync-install.sh",
            file=sys.stderr,
        )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--version", action="version", version=f"jira-cli {CLI_VERSION}")
    ...
    sub.add_parser(
        "doctor",
        help="Show which jira.py runs on PATH and whether this build is up to date",
    )
    ...
    parent_p = sub.add_parser(
        "parent",
        help="Weekly parent issue (Mon–Sat); new logs link as child when issue type allows",
    )
    parent_p.add_argument(
        "tokens",
        nargs="*",
        metavar="[set KEY | clear | KEY]",
        help="set TMD-100, clear, shorthand TMD-100, or omit to show",
    )

    comment_p = sub.add_parser("comment", help="Add a comment to an issue")
    comment_p.add_argument("key", help="Issue key")
    comment_p.add_argument("body", help="Comment text")
...
async def async_main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "parent":
        action, parent_key = parse_parent_tokens(list(args.tokens))
        if action == "clear":
            await cmd_parent(cfg, None, clear=True)
        elif action == "set":
            await cmd_parent(cfg, parent_key, clear=False)
        else:
            await cmd_parent(cfg, None, clear=False)
        return 0
    if args.command == "comment":
        await cmd_comment(cfg, args.key, args.body)
        return 0
    if args.command == "eod":
